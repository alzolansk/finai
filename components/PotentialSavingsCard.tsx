      ...
      <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start md:items-center justify-between">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-indigo-200 font-medium text-sm uppercase tracking-wider">Planejamento Inteligente</span>
          </div>
          ...
          <p className="text-indigo-200/80 text-sm max-w-md">
            {riskCategories.length > 0 
              ? `Atenção: ${riskCategories.length} categorias com risco de estourar o orçamento. Veja oportunidades de economia.`
        ...
        <div className="w-full md:w-auto md:min-w-[320px]">
          <div className="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10">
            <h4 className="text-sm font-bold text-white mb-4 flex items-center justify-between">
              <span>Destaques & Riscos</span>
              <span className="text-xs bg-white/10 px-2 py-1 rounded-full text-indigo-200">{allItems.length} itens</span>
            </h4>
              ...
              {allItems.slice(0, 4).map((item) => (
                <div key={item.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium truncate ${item.type === 'risk' ? 'text-rose-300' : 'text-white'}`}>{item.title}</p>
                    <p className="text-xs text-indigo-200/70 truncate">{item.description}</p>
                  </div>
                  <span className={`text-sm font-bold whitespace-nowrap ${item.type === 'risk' ? 'text-rose-300' : 'text-white'}`}>
